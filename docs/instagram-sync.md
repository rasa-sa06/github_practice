...
4. `alt` 属性を「ぷてぃえーるのヘアリボン作品（◯年◯月◯日の投稿）」に書き換え
...
`alt` は画面に表示されず、読み上げソフトと検索エンジンが読む説明文です。

以前はキャプションの1行目を使っていましたが、投稿は「お久しぶりです」や
絵文字1つで始まることが多く、`alt="🎄"` のような、何の画像か伝わらない
状態になっていました。画像検索にも乗りません。

写真に何が写っているかはこちらでは判定できないため、**確実に正しいことだけ**を
書く方針にしています（作品写真であることと、投稿日）。
キャプションはログにだけ出力し、どの投稿かを確認できるようにしてあります。

## アプリの持ち主について（先に決めること）

Meta のアプリには持ち主がいます。**制作者のアカウントで作り、納品時に依頼者へ移管する**方針で進めています。

| | 制作中 | 納品後 |
|---|---|---|
| アプリの持ち主 | 制作者 | 依頼者へ移管 |
| 依頼者の作業 | 招待の承認と、アクセス許可の2クリックのみ | — |

移管は、アプリの「アプリの役割」から依頼者を**管理者**として追加し、
制作者が自分を外す、という手順で行います。
移管しないままだと、制作者のアカウントを削除したときに連携が切れます。

> **⚠️ トークンをチャットやメールに貼らないでください。** アカウントへのアクセス権を持つ
> 認証情報です。GitHub の Secrets に直接入力してください。
...
**制作者自身のアカウントで作成します**（上記「アプリの持ち主について」参照）。
ユースケースは「Instagram」を選びます。
